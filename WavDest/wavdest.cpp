...
{
	if (encoder) return -1;
	if (aacinfo.samplerate == 0 || aacinfo.channels == 0) return -1;
	...
	encoder = faacEncOpen(aacinfo.samplerate, aacinfo.channels, &framesize, &ob);
	if (!encoder) return -1;

	aacinfo.frame_size  = framesize;
	aacinfo.frames_done = 0;

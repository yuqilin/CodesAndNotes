...
CFlyfoxStreamCtrl::CFlyfoxStreamCtrl()
{
	m_fileSize=0;
	m_available=0;
	m_curPos=0;
...
//0 for success
int CFlyfoxStreamCtrl::Open(const char* name, int file_type)
{
	int ret=0;
	m_type=file_type;
	m_openFileName=name;
...
int CFlyfoxStreamCtrl::Read(void* buffer, unsigned int bytes_to_read, unsigned int* bytes_read)
{
	int ret=0;
	if (m_type==1)
	{
...
//0 for success
int CFlyfoxStreamCtrl::Seek(unsigned __int64 offset)
{
	int ret=0;
	if (m_type==1)
	{
...
int CFlyfoxStreamCtrl::Tell(unsigned __int64* offset)
{
	int ret=0;
	if (m_type==1)
	{
...
int CFlyfoxStreamCtrl::GetSize(unsigned __int64* size)
{
	*size = m_fileSize;
	return 0;
}

int CFlyfoxStreamCtrl::GetAvailable(unsigned __int64* available)
{
	*available=m_available;
	return 0;
}

bool CFlyfoxStreamCtrl::RecvHeaderData(NPT_HttpUrl url,NPT_DataBuffer& buffer)
{
...
bool CFlyfoxStreamCtrl::RecvMediaData(NPT_HttpUrl url,int in_nStartPos, 

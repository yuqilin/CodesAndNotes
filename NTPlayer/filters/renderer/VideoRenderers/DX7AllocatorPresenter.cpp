    ...
    , m_hDDrawLib(NULL)
{
    ...
    if (FAILED(hr = pDirectDrawCreateEx(NULL, (VOID**)&m_pDD, IID_IDirectDraw7, NULL))
            || FAILED(hr = m_pDD->SetCooperativeLevel(hWnd/*AfxGetMainWnd()->GetSafeHwnd()*/, DDSCL_NORMAL))) {
        return;
    }
...
HRESULT CDX7AllocatorPresenter::CreateDevice()
{
    ...
    if (m_pAllocator) {
        ...
        m_pAllocator = new CDX7SubPicAllocator(m_pD3DDev, size, GetRenderersSettings().fSPCPow2Tex);

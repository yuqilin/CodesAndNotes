...
#include <atlbase.h>
#include <atlcoll.h>
#include "../ntplayercore/resource.h"
...
//            DEBUG_ONLY(_tprintf_s(_T("m_pDirect3DCreate9\n")));
    ...
    m_pGenlock = new CGenlock(r.m_AdvRendSets.fTargetSyncOffset, r.m_AdvRendSets.fControlLimit, r.m_AdvRendSets.iLineDelta, r.m_AdvRendSets.iColumnDelta, r.m_AdvRendSets.fCycleDelta, 0); // Must be done before CreateDXDevice
...
//    TRACE(_T("CreateDevice: %d\n"), (LONG)hr);
    ...
    if (m_pAllocator) {
        ...
        m_pAllocator = new CDX9SubPicAllocator(m_pD3DDev, size, GetRenderersSettings().fSPCPow2Tex, false);
        if (!m_pAllocator) {
...
//        DEBUG_ONLY(_tprintf_s(_T("m_pSubPicQueue != NULL\n")));
    ...
    m_pSubPicQueue = GetRenderersSettings().nSPCSize > 0
                     ? (ISubPicQueue*)new CSubPicQueue(GetRenderersSettings().nSPCSize, !GetRenderersSettings().fSPCAllowAnimationWhenBuffering, m_pAllocator, &hr)
...
STDMETHODIMP_(bool) CBaseAP::Paint(bool fAll)
{
    ...
    if (m_pD3DDevEx) {
        if (m_bIsFullscreen) {
            hr = m_pD3DDevEx->PresentEx(NULL, NULL, NULL, NULL, 0);
        } else {
            hr = m_pD3DDevEx->PresentEx(rSrcPri, rDstPri, NULL, NULL, 0);
        }
    } else {
        if (m_bIsFullscreen) {
...
HWND CBaseAP::GetFocusWindow()
{
    if (!m_hFocusWnd) {
        ...
        m_hFocusWnd = CreateWindow(_T("D3DFocusClass"), _T("D3D Focus Window"), WS_OVERLAPPED, 0, 0, 0, 0, NULL, NULL, NULL, NULL);
        if (!m_hFocusWnd) {
...
STDMETHODIMP CSyncAP::CreateRenderer(IUnknown** ppRenderer)
{
    ...
    *ppRenderer = NULL;
    ...
    do {
        ...
        CSyncRenderer* pOuterEVR = new CSyncRenderer(NAME("CSyncRenderer"), pUnk, hr, &m_VMR9AlphaBitmap, this);
...
HRESULT CreateSyncRenderer(const CLSID& clsid, HWND hWnd, bool bFullscreen, ISubPicAllocatorPresenter** ppAP)
{
    ...
    if (clsid == CLSID_SyncAllocatorPresenter) {
        CString Error;
        *ppAP = new CSyncAP(hWnd, bFullscreen, hr, Error);
        (*ppAP)->AddRef();

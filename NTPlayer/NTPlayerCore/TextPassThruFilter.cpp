...
protected:
    void AddSubStream(ISubStream* pSubStream) {
        if (m_pSubStreamOld) {
            if (pSubStream) {
                m_pTPTFilter->m_pGraph->ReplaceSubtitle(m_pSubStreamOld, pSubStream);
            }
    ...
    void InvalidateSubtitle(REFERENCE_TIME rtStart, ISubStream* pSubStream) {
        m_pTPTFilter->m_pGraph->InvalidateSubtitle((DWORD_PTR)pSubStream, rtStart);
    }
...
CTextPassThruFilter::CTextPassThruFilter(DirectShowGraph* pGraph)
    : CBaseFilter(NAME("CTextPassThruFilter"), NULL, this, __uuidof(this))
    , m_pGraph(pGraph)
{
    HRESULT hr;
    m_pInput = new CTextPassThruInputPin(this, this, &m_pGraph->m_csSubLock, &hr);

...
std::wstring CRealTextParser::RenderTags(const std::list<Tag>& p_crlTags)
{
    ...
    std::list<Tag>::const_iterator iter = p_crlTags.begin();
    for (; iter != p_crlTags.end(); ++iter) {
        Tag oTag(*iter);
...
{
    int iCounter(1);
    std::map<std::pair<int, int>, std::wstring>::iterator i;
    for (/*auto */i = m_RealText.m_mapLines.begin(); i != m_RealText.m_mapLines.end(); ++i) {
        p_rOutput << iCounter++;
        p_rOutput << std::endl;

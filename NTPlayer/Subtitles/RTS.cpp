...
bool CWord::CreateOpaqueBox()
{
    ...
    m_pOpaqueBox = new CPolygon(style, str, 0, 0, 0, 1.0, 1.0, 0);
    ...
    : CPolygon(STSStyle(), str, 0, 0, 0, scalex, scaley, 0)
{
    ...
    m_pAlphaMask = new BYTE[size.cx * size.cy];
    if (!m_pAlphaMask) {
        return;
    }
...
CWord* CClipper::Copy()
{
    return new CClipper(m_str, m_size, m_scalex, m_scaley, m_inverse, m_cpOffset);
}
...
void CSubtitle::CreateClippers(CSize size)
{
    ...
    if (m_effects[EF_BANNER] && m_effects[EF_BANNER]->param[2]) {
        ...
        if (!m_pClipper) {
            ...
            m_pClipper = new CClipper(str, size, 1, 1, false, CPoint(0, 0));
            if (!m_pClipper) {
                return;
            }
        }
...
void CRenderedTextSubtitle::ParseString(CSubtitle* sub, CStringW str, STSStyle& style)
{
    ...
    for (int i = 0, j = 0, len = str.GetLength(); j <= len; j++) {
        ...
        if (i < j) {
            if (CWord* w = new CText(style, str.Mid(i, j - i), m_ktype, m_kstart, m_kend, sub->m_scalex, sub->m_scaley)) {
        ...
        if (c == L'\n') {
            if (CWord* w = new CText(style, CStringW(), m_ktype, m_kstart, m_kend, sub->m_scalex, sub->m_scaley)) {
                sub->m_words.AddTail(w);
                m_kstart = m_kend;
            }
        } else if (c == L' ' || c == L'\x00A0') {
            if (CWord* w = new CText(style, CStringW(c), m_ktype, m_kstart, m_kend, sub->m_scalex, sub->m_scaley)) {
                sub->m_words.AddTail(w);
                m_kstart = m_kend;
            }
        }
    ...
    for (int i = 0, j; (j = str.Find(L'\\', i)) >= 0; i = j) {
        ...
        if (cmd == L"1c" || cmd == L"2c" || cmd == L"3c" || cmd == L"4c") {
            ...
            if (params.GetCount() == 1 && !sub->m_pClipper) {
                sub->m_pClipper = new CClipper(params[0], CSize(m_size.cx >> 3, m_size.cy >> 3), sub->m_scalex, sub->m_scaley, invert, (sub->m_relativeTo == 1) ? CPoint(m_vidrect.left, m_vidrect.top) : CPoint(0, 0));
            } else if (params.GetCount() == 2 && !sub->m_pClipper) {
                long scale = wcstol(p, NULL, 10);
                if (scale < 1) {
                    scale = 1;
                }
                sub->m_pClipper = new CClipper(params[1], CSize(m_size.cx >> 3, m_size.cy >> 3), sub->m_scalex / (1 << (scale - 1)), sub->m_scaley / (1 << (scale - 1)), invert, (sub->m_relativeTo == 1) ? CPoint(m_vidrect.left, m_vidrect.top) : CPoint(0, 0));
